...
func GetEndpointByHostAndName(host string, port int) string {
    return fmt.Sprintf("%s:%d", host, port)
}

func GetJobRedisKey(jobId ID)string {
    return fmt.Sprintf("job_%s", jobId)
}
func GetJobAllocRedisKey(jobId ID)string{
    return fmt.Sprintf("accepted_%s", jobId)
}
